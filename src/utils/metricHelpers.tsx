...
export const MetricsNiceName: Record<string, string> = {
  ...
  lsbAdjust: 'Left Side Bearing',
  rsbAdjust: 'Right Side Bearing',
};
...
export const metricDialogData: Record<string, MetricDialogInfo> = {
  ...
  topTrim: {
    ...
    getRawCopy: (state) =>
      state.topTrimRaw
        ? `${state.topTrimRaw}` //`metric-top-trim: ${state.topTrimRaw}, half-leading: ${state.halfLeading}`
        : '-',
    getCss: (state) => (state.topTrimRatio ? `-${state.topTrimRatio}em` : '-'),
    getCssCopy: (state) => {
      if (!state.topTrimRaw || !state.unitsPerEm) return '-';
      const metricTrimEm = (state.topTrimRaw / state.unitsPerEm).toFixed(3);
      return `--top-trim--${state.fontSlug}: calc(-${metricTrimEm}em - ((1lh - 1em) * 0.5));`;
    },
  ...
  bottomTrim: {
    ...
    getCssCopy: (state) => {
      if (!state.bottomTrimRaw || !state.unitsPerEm) return '-';
      const metricTrimEm = (state.bottomTrimRaw / state.unitsPerEm).toFixed(3);
      return `--bottom-trim--${state.fontSlug}: calc(-${metricTrimEm}em - ((1lh - 1em) * 0.5));`;
    },
  ...
  lsbAdjust: {
    ...
    getCssCopy: (state) =>
      state.lsbAdjustRatio
        ...
        : '-',
    info: 'The average Left Side Bearing (LSB) calculated from the 10 most frequently used lowercase letters in English (a, e, h, i, l, m, n, o, r, s). Fonts have built-in spacing to prevent letters from touching. Trimming this gap allows the visible ink to align perfectly flush with the left edge of the container.',
  },
  ...
  rsbAdjust: {
    ...
    getCssCopy: (state) =>
      state.rsbAdjustRatio
        ...
        : '-',
    info: 'The average Right Side Bearing (RSB) calculated from the 10 most frequently used lowercase letters in English (a, e, h, i, l, m, n, o, r, s). Similar to the left adjustment, this removes the built-in spacing on the right side. Trimming this is useful for right-aligned text or for calculating the precise optical width of a text block.',
  },
};
